...
call glslangValidator -DGEN_GRASS=1 -S comp -e main -g -V -o %DataDir%\gen_grass.spv %CodeDir%\grass_shaders.cpp
call glslangValidator -DGBUFFER_VERT=1 -S vert -e main -g -V -o %DataDir%\grass_gbuffer_vert.spv %CodeDir%\grass_shaders.cpp
call glslangValidator -DGBUFFER_FRAG=1 -S frag -e main -g -V -o %DataDir%\grass_gbuffer_frag.spv %CodeDir%\grass_shaders.cpp
...
cl %CommonCompilerFlags% %CodeDir%\grass_demo.cpp -Fmgrass_demo.map -LD /link %CommonLinkerFlags% -incremental:no -opt:ref -PDB:grass_demo_%random%.pdb -EXPORT:Init -EXPORT:Destroy -EXPORT:SwapChainChange -EXPORT:CodeReload -EXPORT:MainLoop
del lock.tmp
call cl %CommonCompilerFlags% -DDLL_NAME=grass_demo -Fegrass_demo.exe %LibsDir%\framework_vulkan\win32_main.cpp -Fmgrass_demo.map /link %CommonLinkerFlags%
